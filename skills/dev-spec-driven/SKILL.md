...
| `/prReview` | Local pre-merge review gated by tracks: spec compliance · +tdd red-first history, every AC tested · +saas tenant isolation (`WHERE tenant_id = ?`), observability, hot-path cost · +ai eval delta in the commit / merge summary, versioned prompts, PII-to-model · security. | — |
