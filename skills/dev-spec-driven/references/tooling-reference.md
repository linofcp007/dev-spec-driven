...
| `spec_next_action` | "You are here → do this next": one ordered `step` (fill → re-review → fix → approve → implement → verify → finish → finished / drift) + `changedSinceApproval` |
...
impact <feature> [--phase requirements|design|tasks] [--reopen]
