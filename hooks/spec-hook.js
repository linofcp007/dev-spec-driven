 ...
 *     feature whose implementing files drifted since finish (bounded; see DRIFT_MAX_FILES).
 *
...
function main(raw) {
  ...
  if (event === "SessionStart") {
    try {
      ...
      // Drift since finish (finished features only). Bounded: over DRIFT_MAX_FILES recorded files (or ~8 MB of them)
      // nothing is hashed — `dev-spec drift` still checks on demand. Never blocks or breaks the session line.
      try {
        const d = spec.drift(pdir, null, { maxFiles: DRIFT_MAX_FILES, maxBytes: 8 * 1024 * 1024 });
