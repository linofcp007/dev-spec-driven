...
- **Record shape** (`.state.json → evidence[<n>]`): the latest run `{command, exitCode, summary, at}` plus
  ...
  are kept in `others`. A note after a run is attached as `note`, never overwrites it. `stale: true` is set by
...
- **Guard mode:** `roadmap.json → meta.guard` (`spec_init {guard}` / `init --guard on|off`, with or without
  ...
  `permissionDecision: "ask"` with a localized reason (a forced tasks approval still counts, with a note).
...
- **Hooks never block and stay cheap.** Every hook exits 0 on any error or irrelevant event, emits at most
  one JSON object, has a 10 s timeout, and only acts on a `.specs/` dev-spec owns. The PostToolUse hook:
