...
`spec_depend` MCP tool with the feature, its `dependsOn` list, and/or an explicit `order` (CLI:
`dev-spec depend <feature> [deps...]`, or `--add` / `--rm` to change single dependencies and `--clear`
to drop them all). It writes
