...
- **Fails** (block advancing): `ears` errors, `clarifications` still open, `ac-uniqueness`, `placeholders` (template
  ...
  a `[SaaS]`/`[AI]` section, which the design approval refuses; only a bracket whose text the templates write, or
  TODO / TBD / FIXME / `…`, is a placeholder — real values such as `[owner, admin]` or `[free: 60, pro: 600]` are content), `traceability` gaps (a phantom AC that a recorded change request removed is named
