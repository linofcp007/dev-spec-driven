...
`spec_complete_task {evidence}` (the `_Verify:_` command and its exit code — a note alone leaves a runnable
`_Verify:_` unverified, a failed run is recorded and refuses the tick) when each goes green. With `--subagents`, follow
