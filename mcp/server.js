...
const TOOLS = [
  ...
  },
  {
    ...
    description: "Mark task N as done in a feature's tasks.md — ticks exactly that task's checkbox line, found by the same comment- and fence-aware scanner as status ('01' is task 1; a duplicated number resolves to its first OPEN task) — and return updated progress + the new next task. Pass `evidence` — the verification you actually ran: {command, exitCode, summary} (the task's _Verify:_ command, its exit code, an output summary). Every run is recorded in .state.json evidence[N] (the latest run plus a short history); a non-zero exitCode REFUSES the tick and stays recorded (a failed re-check of a ticked task makes it unverified; only a later passing run clears it). An exit code alone, or a command without its exit code, is rejected. EVIDENCE GATE: a task whose _Verify:_ names a runnable command counts as verified only with {command, exitCode: 0} — a summary-only note ticks it but leaves it unverified (a task without a runnable _Verify:_ may be attested by a note). The result carries `verified`; when a task with a runnable _Verify:_ (or with recorded evidence) is not verified it also carries a stable `unverifiedReason` (no-evidence · failed-run · manual-note-on-runnable-verify · duplicate-number · stale-evidence — e.g. spec_impact reopened the task, or its _Verify:_ changed) plus a localized note; doctor and spec_finish keep listing such a task with a localized reason, and ROADMAP.md counts it. Bugfix: a task after the root-cause task is refused (nothing recorded or ticked) until bug.md's Root Cause is filled. Evidence can also be back-filled for a task ticked earlier.",
  ...
  },
  {
    ...
    description:
      "\"You are here, do this next.\" ONE recommendation for a feature, chosen in the spec chain's order — `step`: fill (the first chain artifact still missing or a template; `file` names it) → re-review (an artifact changed after ITS approval, so a spec edited post-approval is re-reviewed, not silently shipped; `impact` = {tool: 'spec_impact', phases} when that approval has a snapshot to diff) → fix (failing checks of the current or an earlier phase — or, when the approve gate would refuse the next pending approval, what it fails on: `refusedGate` {phase, failing}) → approve (the first pending gate) → implement (the next open task) → finish (every task done: run spec_finish); `tasks` when there are no tasks yet. Also returns phase, tracks, the doctor verdict, gatesOk, pendingGates, changedSinceApproval and the localized `recommendation`. Use to resume work or answer 'what now?'.",
    inputSchema: { type: "object", properties: { name: { type: "string" }, projectDir: { type: "string" } }, required: ["name"] },
  },
  {
    name: "spec_add_track",
